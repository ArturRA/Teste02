...
class Chamado:
	...
	def register_chamado(empresa, chamados):
		...
		titulo_do_chamado = Chamado.validar_titulo_do_chamado()

		# 2. Validar descricao do chamado
		descricao_do_chamado = Chamado.validar_descricao_do_chamado()
	
		# 3. Validar id do equipamento
		id_do_equipamento = Chamado.validar_id_do_equipamento(empresa)

		# 4. Validar data de fabricacao
		data_de_abertura = Chamado.validar_data_de_abertura()
	...
	def check_chamados(empresa, chamados):
		if len(chamados) == 0:
			...
			for x in range(len(chamados)):
				...
				print(str(x + 1) + ': Titulo do chamado: ' + chamados[x].titulo_do_chamado + Maquina.check_info_machine_chamado(empresa, chamados[x].id_do_equipamento) +
					  '; descricao do chamado: ' + chamados[x].descricao_do_chamado + '; dias que o chamado esta aberto: ' + Chamado.dias_chamado_aberto(chamados[x].data_de_abertura))
	...
	def edit_chamados(chamados):
		if len(chamados) == 0:
			...
			Chamado.check_chamados(chamados)
			chamado_para_editar = ''
			while True:
		...
		while True:
			...
			match operacao_de_edicao:
				...
				case '2':
					...
					continue
				case '3':
					id_do_equipamento = Chamado.validar_id_do_equipamento()
